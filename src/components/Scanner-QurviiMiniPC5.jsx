...
const QrScanner = () => {
  ...
  return (
    ...
    <div className="flex flex-col items-center sm:w-100 w-[90vw]  justify-center sm:min-h-[50vh] min-h-[80vh]  rounded sm:shadow sm:mt-10 mx-auto p-4">
      ...
      {error && (
        <div className={`bg-red-100 text-red-700 p-3 rounded w-full max-w-md text-center mb-4 ${depart===""?"hidden":"block"}`}>
          {error}
        </div>
      )}
      ...
      {mode === "camera" && !scannedData && showScanner && (
        <div
          id={qrRegionId}
          className={`${depart===""?"hidden":"block"} w-64 h-64 rounded border border-gray-200 mb-4`}
        ></div>
      )}
